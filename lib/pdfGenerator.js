...
// ── Font registration (base-14 PDF fonts — no VFS needed) ─────────────────────
const fonts = {
  Helvetica: {
    normal:      'Helvetica',
    bold:        'Helvetica-Bold',
...
function buildOwnerDetails(d) {
  ...
  return tbl([t, t, t3], [
    ...
    [
      ...
      cell('WhatsApp No.',      d.whatsapp),
      cell('Email Address',     d.email),
    ],
    [cell('Permanent Address (if different from this unit)', d.perm_address, C.LIGHT, { colSpan: 3 }), {}, {}],
  ]);
}
...
function buildTenantDetails(d) {
  ...
  return tbl([a, b, c], [
    sectionHdr('04', 'TENANT DETAILS  (IF RENTED)', 3),
    [
...
function buildDocuments(d) {
  const h   = Math.floor(W / 2); // 270
  const ch1 = d.doc_tenant_agreement ? '[X]' : '[ ]';
  const ch2 = d.doc_pet_cert         ? '[X]' : '[ ]';

  return tbl([h, W - h], [
    sectionHdr('08', 'DOCUMENTS ENCLOSED — CHECKLIST', 2),
    [
