// Strip BOM (﻿) and any stray whitespace/newlines PowerShell injects into env vars
const SCRIPT_URL = (process.env.GOOGLE_SCRIPT_URL || '')
  .replace(/^﻿/, '')   // UTF-8 BOM
  .replace(/\r\n|\r|\n/g, '') // CRLF / LF line endings
  .trim();
...
export async function deleteSubmission(unitNumber) {
  checkConfig();
  const res = await fetch(SCRIPT_URL, {
    method: 'POST',
    redirect: 'follow',
    headers: { 'Content-Type': 'text/plain' },
    body: JSON.stringify({ action: 'deletesubmission', unit: unitNumber }),
  });
