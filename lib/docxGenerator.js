...
function buildPets(d) {
  const pw = (() => {
    const cols = [
      Math.floor(W * 0.25), Math.floor(W * 0.10), Math.floor(W * 0.18),
      Math.floor(W * 0.22), 0,
    ];
  ...
  const petRows = Array.from({ length: 5 }, (_, i) => {
    const n = i + 1;
    return new TableRow({ children: [
      tc(pw[0], '', d[`pet_${n}_breed`],       { shade: C.WHITE }),
      tc(pw[1], '', d[`pet_${n}_age`],         { shade: C.WHITE }),
      tc(pw[2], '', d[`pet_${n}_vaccinated`],  { shade: C.WHITE }),
      tc(pw[3], '', d[`pet_${n}_vacc_date`],   { shade: C.WHITE }),
      tc(pw[4], '', d[`pet_${n}_cert_status`], { shade: C.WHITE }),
    ]});
  });

  return makeTable(pw, [
    sectionHdrRow('06', 'PET DETAILS & VACCINATION', 5, [pw[0]]),
    new TableRow({ children: [
      new TableCell({
        width: { size: W, type: WidthType.DXA },
        columnSpan: 5,
    ...
    new TableRow({ children: [
      hdrCell(pw[0], 'Type / Breed'),
      hdrCell(pw[1], 'Age'),
      hdrCell(pw[2], 'Vaccinated?'),
      hdrCell(pw[3], 'Last Vacc. Date'),
      hdrCell(pw[4], 'Certificate Status'),
    ]}),
    ...petRows,
  ]);
}
